 ...
 * 游戏阶段类型
 */
export type GamePhase = 'lobby' | 'p1_draw' | 'p1_action' | 'p2_draw' | 'p2_action' | 'p1_select' | 'p2_select' | 'scoring' | 'game_over';
...
/** 检查是否为赠予行动 */
export declare function isGiftAction(action: PendingAction): action is GiftPending;
/** 检查是否为竞争行动 */
export declare function isCompetitionAction(action: PendingAction): action is CompetitionPending;
/** 安全获取赠予行动的卡牌列表 */
export declare function getGiftCards(action: PendingAction): string[] | null;
/** 安全获取竞争行动的卡牌分组 */
export declare function getCompetitionCards(action: PendingAction): string[][] | null;
