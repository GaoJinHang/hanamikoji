 ...
 */
import type { ActionType, GameState, PendingAction, PlayerId } from '@hanamikoji/shared';

export interface EngineMeta {
  /**
   * Legacy orchestration flag.
   * Reducer now auto-creates the next round by default; this flag remains for backwards-compatible
   * APPLY_ROUND_SETUP flows and external replay tooling.
   */
  needsRoundSetup: boolean;
}

export interface EngineState {
  /** Full authoritative game state. Server must mask it with createPlayerView before sending to clients. */
  gameState: GameState;
  /** Remaining deck card ids after dealing (hidden card already removed). */
  deck: string[];
  /** Internal meta flags for server orchestration (still pure). */
  meta: EngineMeta;
  /** Deterministic RNG state used for replayable next-round setup. */
  rngState: number;
}
...
export type EngineAction =
  ...
  | { type: 'APPLY_ROUND_SETUP'; deck: string[]; hands: { p1: string[]; p2: string[] }; rngState?: number }
